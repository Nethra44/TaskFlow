...
export default function Today({ tasks, onDelete, onToggle, onExtend }) {
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    return isToday && matchPriority && matchCategory;
  });
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div>
          ...
          <p className="text-slate-400 font-medium mt-1">
            Focus for {new Date().toLocaleDateString()}
          </p>
        </div>
        ...
        {filteredTasks.length === 0 ? (
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col items-center justify-center p-20 min-h-400px">
            ...
            <h3 className="text-xl font-bold text-slate-400 tracking-tight text-center">
              No tasks due today. Enjoy your clear schedule!
            </h3>
          </div>
        ) : (
          filteredTasks.map((t) => (
      ...
      <div className="flex gap-1 px-1">
        {options.map((opt) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${active === opt ? "bg-slate-800 text-white shadow-sm" : "text-slate-500 hover:bg-slate-50"}`}
          >
            {opt}
          </button>
        ))}
